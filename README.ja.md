...
> [!NOTE]
> ここで用いる対話ログは、ツイネージュだけではなく、`ChatGPT` や `Claude` などのクラウドAIの対話ログを用いることができます。

### 抽出前の準備（環境変数の設定）
抽出器は、対話ログ内の太字タグ（例: `**ChatGPT**`）を目印にしてAIとユーザーの発言ターンを特定します。ログを正しく読み込ませるため、抽出を実行する前に `.env` ファイルへ以下の環境変数を設定してください。

*   **`TWINAGE_ASSISTANT_NAME` (必須)**: 対話したAIの名前を指定します（例: `ChatGPT`, `Claude`, `Gemini`, `Assistant` など）。未設定の場合はエラーとなり起動しません。
*   **`TWINAGE_USER_NAME` (任意)**: あなた（ユーザー）の名前を指定します。未設定の場合、デフォルトは `You` として処理されます。

> [!WARNING]
> 指定した名前のタグがマークダウン内に1つも見つからない場合、抽出器は有効なターンがないと判断し、処理をスキップします。ファイル内の表記と環境変数が完全に一致しているか確認してください。
