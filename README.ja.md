...
本リポジトリは、思考の抽出、対話UI、そして三値で判断する判定ノードに至るまで、プラットフォームとしての最小構成を提供します。システムは独立したマイクロサービス・アーキテクチャを採用しており、純粋な記録の引き出しを担う「L1（インフラ層）」と、その記憶を利用する「L2（アプリケーション層）」に分離されています。
...
作成した `.env` をテキストエディタで開き、利用する環境に合わせて設定します。

```sh
# クラウドAPIを利用する場合は有効なキーが必須です
OPENAI_API_KEY=sk-...
```
...
WebUIや各種エージェントの土台となる、データベース検索専用のコアAPIを起動します。別のターミナルを開いて実行してください。
...
L1 APIが起動している状態で、元のターミナルからWebUIを起動します。

```bash
uv run chainlit run twinage/webui.py
```

自動でWebブラウザが `127.0.0.1:8000` を開きます。まずは「ツイネージュって何？」と尋ねてみてください。
...
## 高度な利用：判定ノード（L2 Judgment API）

Twinageは外部の自律エージェント（Dify等）の意思決定を制御する「判定ノード」としても機能します。

以下のコマンドで、L1を裏側で利用する判定用APIを起動できます（ポート8083）。

```bash
uv run uvicorn twinage.api.L2.judgement:app --port 8083
```

外部システムからこのエンドポイントに対して `{"proposal": "〇〇したい"}` という行動計画をPOSTすると、Twinageは過去の記憶と哲学に照らし合わせ、その行動が「ACCEPT（受容）」か「REJECT（棄却）」または「UNKNOWN（判断不可）」かを厳密なJSONで回答します。これにより、パブリッシャー側に自由な開発を許可しつつ、システムの思想的整合性を保つことが可能です。
